...
#     plate_fn <- glue('{cgdb@db_root}/{plate_name}.h5')
#     if (file.exists(plate_fn)){
#         file.remove(plate_fn)
#     }

    
#     if (nrow(anti_join(smat$intervs %>% select(chrom, start, end), cgdb@cpgs %>% select(chrom, start, end), by=c('chrom', 'start', 'end'))) != 0){
#         smat <- smat.add_intervals(smat, cgdb@cpgs %>% select(chrom, start, end))   
#     }
...
#     cells <- smat$cell_metadata %>% 
#         left_join(
#             tibble(cell_id = colnames(smat)) %>% 
#             mutate(j = 1:n())) %>% 
#         separate(cell_id, c('plate1', 'cell_num'), sep='\\.', remove=FALSE) %>% 
#         select(-plate1) %>% 
#         mutate(cell_num = as.numeric(cell_num)) %>% 
#         arrange(cell_num) %>% 
#         mutate(bucket = ntile(cell_num, n() / 100))

#     h5f <- h5file(plate_fn, mode='a')
#     on.exit(h5close(h5f))

#     x <- cells %>% filter(bucket == 1)
#     createDataSet(h5f, datasetname='meth', data=as.matrix(smat$meth[, x$j]), chunksize=c(round(nrow(smat$meth) / 300), 1), maxdimensions=NA_integer_, compression=9L, size=-1)
#     createDataSet(h5f, datasetname='cov', data=as.matrix(smat$cov[, x$j]), chunksize=c(round(nrow(smat$cov) / 300), 1), maxdimensions=NA_integer_, compression=9L, size=-1)
#     h5flush(h5f)
#     rm(x)
    
#     dummy <- cells %>% 
#         filter(bucket > 1) %>% 
#         plyr::dlply(plyr::.(bucket), 
#             function(x) {
#                 h5f['meth'] <- cbind(h5f['meth'], as.matrix(smat$meth[, x$j])) 
#                 h5f['cov'] <- cbind(h5f['cov'], as.matrix(smat$cov[, x$j])) 
#                 h5flush(h5f)
#             }, .progress = 'text')

#     h5f['cell_id'] <- cells$cell_id
#     h5close(h5f)

#     # cgdb@plates[[plate_name]] <- h5file(plate_fn, mode='r')

#     return(cgdb)
# }


# get_h5_obj <- function(cgdb, plate){
#     plate_fn <- glue('{cgdb@db_root}/{plate}.h5')
#     if (!file.exists(plate_fn)){
#         stop('plate {plate} doesn\'t exist!')
#     }

#     if (is.null(cgdb@plates[[plate]])){
#         cgdb@plates[[plate]] <- h5file(plate_fn, mode='r')
#     }

#     return(cgdb@plates[[plate]])
# }
