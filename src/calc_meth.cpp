...
DataFrame mean_meth(IntegerVector const& idxs, std::string const& db_dir, std::vector<std::string> const& cells, const int& CPG_NUM)
{
    ...
    // std::vector<float> met_isct(idxs.length());
    ...
    for (unsigned i=0; i<cells.size(); ++i) {
        ...
        std::string idx_fname = fname + ".idx_c.bin";
        std::string met_fname = fname + ".meth_c.bin";
        ...
        if ((cell_idx == NULL) || (cell_met == NULL)) {
            ...
            cov[i] = cblas_sdoti(ncpgs, &ones[0], cell_idx, &mask[0]);
        }
